...
export default function CreateArticle() {
    ...
    const handleSave = async () => {
        try {
            ...
            const headerImageBase64 = data.mainImgFile
                ? await toBase64(data.mainImgFile)
                : DEFAULT_BASE64_IMAGE;
            const contentImageBase64 = data.contentImgFile
                ? await toBase64(data.contentImgFile)
                : DEFAULT_BASE64_IMAGE;
            ...
            if (!res.ok)
                throw new Error(
                    result?.message || "حدث خطأ أثناء إنشاء المقال"
                );
                        ...
                        <div className='border rounded-md p-3 w-full relative'>
                            ...
                            <label className='absolute top-2 right-2 bg-gray-200 hover:bg-gray-300 cursor-pointer h-9 rounded-md px-3 flex items-center gap-2 text-sm font-medium'>
                                ...
                                <input
                                    ...
                                    className='hidden'
                                    onChange={(e) =>
                                        handleImageChange(e, "contentImg")
                                    }
                                />
                            </label>
                        </div>
